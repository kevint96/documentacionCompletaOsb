...
def recorrer_servicios_internos_osb(project_path,operacion_a_documentar, pipeline_path, operations, visited_proxies=None):
    ...
    services_for_operations = definir_operaciones_internas_pipeline(pipeline_path)
    
    
    return services_for_operations

def procesar_elemento(elemento, namespaces, services_for_operations, service_refs):
    service_element = elemento.find(".//con1:service", namespaces=namespaces)
    st.success(f"🔎 service_element: {service_element}")
    operation_element = elemento.find(".//con1:operation", namespaces=namespaces)
    ...
    try:
        with open(pipeline_path, 'r', encoding="utf-8") as f:
            ...
            namespaces = {
                ...
                'con1': 'http://www.bea.com/wli/sb/stages/config',
                'con2': 'http://www.bea.com/wli/sb/stages/routing/config',
                'con3': 'http://www.bea.com/wli/sb/stages/transform/config',
                'con4': 'http://www.bea.com/wli/sb/stages/publish/config',
                'ref': 'http://www.bea.com/wli/sb/reference',
                'xsi': 'http://www.w3.org/2001/XMLSchema-instance'
            ...
            elementos_buscar = [
                (".//con3:wsCallout", namespaces),
                (".//con1:wsCallout", namespaces),
                (".//con3:javaCallout", namespaces),
                (".//con4:route", namespaces),
                (".//con2:route", namespaces)
            ]
            
            for xpath, ns in elementos_buscar:
                for elemento in root.findall(xpath, namespaces=ns):
                    st.success(f"🔎 elemento: {elemento}")
                    st.success(f"🔎 ns: {ns}")
                    st.success(f"🔎 services_for_operations: {services_for_operations}")
                    st.success(f"🔎 service_refs: {service_refs}")
                    procesar_elemento(elemento, ns, services_for_operations, service_refs)
            
            # Procesar elementos dentro de <con:flow>
            for flow_element in root.findall(".//con:flow", namespaces):
                for service_element in flow_element.findall(".//con1:service[@xsi:type='ref:BusinessServiceRef']", namespaces):
                    service_ref = service_element.attrib.get('ref', '')
                    st.success(f"🔎 service_ref: {service_ref}")
                    operation_elements = flow_element.findall(".//con1:operation", namespaces)
                    st.success(f"🔎 operation_elements: {operation_elements}")
                    operation_name = operation_elements[0].text.strip() if operation_elements else service_ref.split('/')[-1]
                    st.success(f"🔎 operation_name: {operation_name}")
                    new_operation_name = operation_name
                    version = 2
                    while new_operation_name in services_for_operations and services_for_operations[new_operation_name] != service_ref:
        ...
        st.error(f"El archivo {pipeline_path} no pudo ser encontrado.")
